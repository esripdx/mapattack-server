...
function get_team_for_game(device_id, game_id, callback) {
  redis.multi()
    .sismember("game:"+game_id+":red", device_id)
    .sismember("game:"+game_id+":blue", device_id)
...
function set_device_team(device_id, game_id, team, callback) {
  var otherteam;
  if(team == "red") {
    otherteam = "blue";
  } else {
  ...
  redis.multi()
    .set("device:team:"+device_id, team)
    .srem("game:"+game_id+":"+otherteam, device_id)
    .sadd("game:"+game_id+":"+team, device_id)
    .exec(function(err, replies){
...
function get_team_counts(game_id, callback) {
  redis.multi()
    .scard("game:"+game_id+":red")
    .scard("game:"+game_id+":blue")
    .exec(function(err, replies){
      callback(err, {
        red: replies[0],
        blue: replies[1]
...
function get_players(game_id, callback) {
  redis.multi()
    .smembers("game:"+game_id+":red")
    .smembers("game:"+game_id+":blue")
    .exec(function(err, replies){
      callback(err, {
        red: replies[0],
        blue: replies[1]
...
exports.device = {
  ...
  set_team: set_device_team,
  get_team: get_device_team,
...
exports.get_team_counts = get_team_counts;
