      ...
      session.get_nearby_triggers_for_tag("board", {
        ...
        if(data) {
          data.forEach(function(e){
            var distance, polygon;
            var user_location = new Terraformer.Point([request.query.longitude, request.query.latitude]);

            if(e.condition.geo.geojson) {
              polygon = new Terraformer.Polygon(e.condition.geo.geojson);
            ...
            } else if(e.condition.geo.distance) {
              polygon = new Terraformer.Circle([e.condition.geo.longitude, e.condition.geo.latitude], e.condition.geo.distance, 32);

              if(user_location.within(polygon)) {
                distance = 0;
              } else {
                distance = Math.round(geo.gc_distance(user_location, new Terraformer.Point([e.condition.geo.longitude, e.condition.geo.latitude])));
              }
            }
