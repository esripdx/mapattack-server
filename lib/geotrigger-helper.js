...
function set_active_game (game_id) {
  (function(device_id){
    // Find the previously active game
    redis.device.get_active_game(device_id, function(err, game_id){
      // Remove the device from the previous game
      redis.device.remove_device_from_game(device_id, game_id, function(err,data){});
    });
    // Add the device to the new game
    redis.device.add_device_to_game(device_id, game_id, function(err,data){});

    // Set the current active game for the device
    console.log("Setting active game for device:"+device_id + " to game:"+game_id);
    redis.device.set_active_game(device_id, game_id, function(err, data){});

  })(this.device_id);
}

exports.set_active_game = set_active_game;

function set_udp_info (address, port) {
  console.log("Saving UDP port info for device: "+this.device_id + " to "+address+":"+port);
