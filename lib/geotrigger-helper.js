...
function get_triggers_for_tag (device_id, tag, geo_param, callback) {
...
function new_session(device_id, response, callback) {
  get_tokens(device_id, function(err, tokens){
    if(err || tokens.access_token == null) {
      ...
      var session = exports;
      session.geotrigger = create_geotrigger_session(device_id, tokens);
      callback(session);
    }
  });
}
