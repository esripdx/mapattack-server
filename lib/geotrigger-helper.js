...
function set_active_game (game_id) {
  console.log("Setting active game for device:"+this.device_id + " to game:"+game_id);
  redis.device.set_active_game(this.device_id, game_id, function(err, data){});
}
...
function publish_location (location) {
  ...
  (function(device_id){
    redis.device.get_active_game(device_id, function(err, game_id){
      if(err) {
        ...
        redis.device.publish_location(device_id, game_id, location);
      }
    });
  })(this.device_id);
}
